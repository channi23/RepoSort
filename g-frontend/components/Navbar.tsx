...
// Removed Link import since we are handling state on the same page
import github from "@/assets/github.svg";

// Define props to accept the login handler
type NavbarProps = {
  onSignIn?: () => void;
};
...
export default function Navbar({ onSignIn }: NavbarProps) {
  return (
    <nav className="w-full h-20 bg-neutral-950 flex items-center justify-between px-10">
      ...
      {/* Changed logic: onClick triggers the passed onSignIn function */}
      <button 
        onClick={onSignIn}
        className="flex items-center gap-3 bg-red-500 border-2 border-red-500 rounded-xl px-5 py-2.5 hover:bg-transparent transition-all duration-200 group"
      >
